...
export default function TodosBox({ todos }: Props) {
  const { execute, result, optimisticState } = useOptimisticAction( // [!code highlight]
    addTodo, // [!code highlight]
    { // [!code highlight]
      currentState: { todos }, // gets passed from Server Component // [!code highlight]
      updateFn: (state, newTodo) => { // [!code highlight]
        return { // [!code highlight]
          todos: [...state.todos, newTodo] // [!code highlight]
        }; // [!code highlight]
      } // [!code highlight]
    } // [!code highlight]
  ); // [!code highlight]
    ...
    <div>
      <button
        onClick={() => {
          ...
          execute({ id: crypto.randomUUID(), body: "New Todo", completed: false }); // [!code highlight]
        }}>
        Add todo
      </button>
