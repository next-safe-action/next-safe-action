...
description: Getting started with next-safe-action.
---
  ...
  return (
    <button
      onClick={async () => {
        ...
        const res = await loginUser({ // [!code highlight]
          username: "johndoe", // [!code highlight]
          password: "123456", // [!code highlight]
        }); // [!code highlight]
