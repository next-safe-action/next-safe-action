...
}).use(async ({ next, clientInput, metadata }) => { // [!code highlight]
  console.log("LOGGING MIDDLEWARE"); // [!code highlight]
 // [!code highlight]
  const startTime = performance.now(); // [!code highlight]
 // [!code highlight]
  // Here we await the action execution. // [!code highlight]
  const result = await next(); // [!code highlight]
 // [!code highlight]
  const endTime = performance.now(); // [!code highlight]
 // [!code highlight]
  console.log("Result ->", result); // [!code highlight]
  console.log("Client input ->", clientInput); // [!code highlight]
  console.log("Metadata ->", metadata); // [!code highlight]
  console.log("Action execution took", endTime - startTime, "ms"); // [!code highlight]
 // [!code highlight]
  // And then return the result of the awaited action. // [!code highlight]
...
export const authActionClient = actionClient
  ...
  .use(async ({ next }) => { // [!code highlight]
    const cookieStore = await cookies(); // [!code highlight]
    const session = cookieStore.get("session")?.value; // [!code highlight]
 // [!code highlight]
    if (!session) { // [!code highlight]
      throw new Error("Session not found!"); // [!code highlight]
    } // [!code highlight]
 // [!code highlight]
    const userId = await getUserIdFromSessionId(session); // [!code highlight]
 // [!code highlight]
    if (!userId) { // [!code highlight]
      throw new Error("Session is not valid!"); // [!code highlight]
    } // [!code highlight]
 // [!code highlight]
    // Return the next middleware with `userId` value in the context // [!code highlight]
    return next({ ctx: { userId } }); // [!code highlight]
  }); // [!code highlight]
```
...
import { authActionClient } from "@/lib/safe-action"; // [!code highlight]
import { z } from "zod";
...
const editProfile = authActionClient
  ...
  .action(async ({ parsedInput: { newUsername }, ctx: { userId } }) => { // [!code highlight]
...
const deleteUser = authActionClient
  .use(async ({ next, ctx }) => { // [!code highlight]
    // `userId` comes from the context set in the previous middleware function. // [!code highlight]
    const userRole = await getUserRole(ctx.userId); // [!code highlight]
 // [!code highlight]
    if (userRole !== "admin") { // [!code highlight]
      throw new ActionError("Only admins can delete users."); // [!code highlight]
    } // [!code highlight]
 // [!code highlight]
    // Here we pass the same untouched context (`userId`) to the next function, since we don't need // [!code highlight]
    // to add data to the context here. // [!code highlight]
    return next(); // [!code highlight]
  }) // [!code highlight]
...
export const testAction = actionClient
  .action(async ({ ctx }) => {
    ...
    const { sessionId, userId } = ctx; // [!code highlight]
  });
```
...
const myMiddleware1 = createMiddleware().define(async ({ next }) => { // [!code highlight]
  // Do something useful here... // [!code highlight]
  return next({ ctx: { baz: "qux" } }); // [!code highlight]
}); // [!code highlight]
...
const myMiddleware2 = createMiddleware<{ // [!code highlight]
  ctx: { foo: string }; // [1] // [!code highlight]
  metadata: { actionName: string }; // [2] // [!code highlight]
  serverError: { message: string } // [3] // [!code highlight]
}>().define(async ({ next }) => { // [!code highlight]
  // Do something useful here... // [!code highlight]
  return next({ ctx: { john: "doe" } }); // [!code highlight]
}); // [!code highlight]
