...
export const loginUser = actionClient
  .inputSchema(inputSchema, {
    ...
    handleValidationErrorsShape: async (ve, utils) => flattenValidationErrors(ve).fieldErrors, // [!code highlight]
  })
...
const inputSchema = z
  ...
  .refine(({ password, confirmPassword }) => password === confirmPassword, { // [!code highlight]
    path: ["confirmPassword"], // [!code highlight]
    message: "Passwords do not match", // [!code highlight]
  }); // [!code highlight]
```
...
const signupAction = actionClient
  ...
  .action(async ({ parsedInput: { email } }) => {
    ...
    if (!isEmailAvailable(email)) {
      returnValidationErrors(schema, { // [!code highlight]
        email: { // [!code highlight]
          _errors: ["Email already registered"], // [!code highlight]
        }, // [!code highlight]
      }); // [!code highlight]
    }
