...
export const statelessFormAction = actionClient
  ...
  .action(async ({ parsedInput }) => { // [!code highlight]
    await new Promise((res) => setTimeout(res, 1000)); // [!code highlight]
 // [!code highlight]
    return { // [!code highlight]
      newName: parsedInput.name, // [!code highlight]
    }; // [!code highlight]
  }); // [!code highlight]
```
...
export default function StatelessForm() {
  const { execute } = useAction(statelessFormAction); // [!code highlight]
...
export const statefulFormAction = action
  .inputSchema(inputSchema)
  // Note that we need to explicitly give a type to `stateAction` here, // [!code highlight]
  // for its return object. This is because TypeScript can't infer the // [!code highlight]
  // return type of the function and then "pass it" to the second argument // [!code highlight]
  // of the server code function (`prevResult`). If you don't need to // [!code highlight]
  // access `prevResult`, though, you can omit the type here, since it // [!code highlight]
  // will be inferred just like with `action` method. // [!code highlight]
  .stateAction<{ // [!code highlight]
    prevName?: string; // [!code highlight]
    newName: string; // [!code highlight]
  }>(async ({ parsedInput }, { prevResult }) => { // [!code highlight]
    return { // [!code highlight]
      prevName: prevResult.data?.newName, // [!code highlight]
      newName: parsedInput.name, // [!code highlight]
    }; // [!code highlight]
  }); // [!code highlight]
```
...
export default function StatefulForm() {
  ...
  const [state, action, isPending] = useActionState( // [!code highlight]
    statefulFormAction, // [!code highlight]
    { // [!code highlight]
      data: { newName: "jane" }, // optionally pass initial state // [!code highlight]
    } // [!code highlight]
  ); // [!code highlight]

  return (
    <form action={action}> {/* [!code highlight] */}
      <input type="text" name="name" placeholder="Name" />
      <button type="submit">Submit</button>
    </form>
  );
