...
Add a validated middleware function that runs **after** input validation. Only available after `inputSchema()` or `bindArgsSchemas()` has been called. Cannot be followed by `inputSchema()` or `bindArgsSchemas()`.
...
<TypeTable
	type={{
		middlewareFn: {
			description: "A validated middleware function that receives parsed inputs and must call next() to continue the chain.",
			type: "ValidatedMiddlewareFn<ServerError, Metadata, Ctx, Ctx & NextCtx, ParsedInput, ClientInput, BindArgsParsedInputs, BindArgsClientInputs>",
		},
	}}
/>
...
<TypeTable
	type={{
		parsedInput: {
			description: "Validated input data (output type of the input schema, including transforms).",
			type: "InferOutput<InputSchema>",
		},
		clientInput: {
			description: "Raw input from the client, before validation (input type of the schema).",
			type: "InferInput<InputSchema>",
		},
		...
		ctx: {
			description: "Context object from previous middleware (both use() and earlier useValidated()).",
			type: "Ctx",
		},
		metadata: {
			description: "Action metadata set via .metadata().",
		...
		next: {
			description: "Call to proceed to the next validated middleware or server code. Pass { ctx } to extend context.",
			type: "(opts?: { ctx?: NextCtx }) => Promise<MiddlewareResult>",
		},
	}}
/>

**Returns:** A new `SafeActionClient` with `NextCtx` merged into the context type. After calling `useValidated()`, `inputSchema()` and `bindArgsSchemas()` are no longer callable (TypeScript error).
...
Methods can be called in any order, with just one constraint: the final one must be `.action()` / `.stateAction()`.
...
- [`createMiddleware()` / `createValidatedMiddleware()`](/docs/api/create-middleware): standalone middleware factories
